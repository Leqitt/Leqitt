...
class Particle {
    constructor(x, y, radius, color) {
      ...
      this.color = color;
      this.velocity = {
        x: (Math.random() - 0.5) * 5, // Random initial velocity
        y: (Math.random() - 0.5) * 5 // Random initial velocity
      };
    }
  ...
  document.getElementById('solid').addEventListener('click', () => {
    particles = [];
    addParticles(100); // Add more particles for solids
  });
  
  document.getElementById('liquid').addEventListener('click', () => {
    particles = [];
    addParticles(50); // Fewer particles for liquids
  });
  
  document.getElementById('gas').addEventListener('click', () => {
    particles = [];
    addParticles(20); // Even fewer particles for gases
  });
